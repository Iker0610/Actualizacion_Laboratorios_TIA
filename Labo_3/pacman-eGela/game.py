    ...
    def asList(self, key=True):
        ...
        for x in range(self.width):
            for y in range(self.height):
                if self[x][y] == key: point_list.append((x, y))
        return point_list
...
class GameStateData:
    ...
    def __str__(self):
        ...
        for agentState in self.agentStates:
            ...
            x, y = nearestPoint(agentState.configuration.pos)
...
class Game:
    ...
    def run(self):
        ...
        for agent_indx in range(len(self.agents)):
            agent = self.agents[agent_indx]
            if not agent:
                self.mute(agent_indx)
                # this is a null agent, meaning it failed to load
                # the other team wins
                print(f"Agent {agent_indx} failed to load", file=sys.stderr)
                self.unmute()
                self._agentCrash(agent_indx, quiet=True)
                return
            if "registerInitialState" in dir(agent):
                self.mute(agent_indx)
                if self.catchExceptions:
                    try:
                        timed_func = TimeoutFunction(agent.registerInitialState, int(self.rules.getMaxStartupTime(agent_indx)))
                        try:
                            start_time = time.time()
                            timed_func(self.state.deepCopy())
                            time_taken = time.time() - start_time
                            self.totalAgentTimes[agent_indx] += time_taken
                        except TimeoutFunctionException:
                            print(f"Agent {agent_indx} ran out of time on startup!", file=sys.stderr)
                            self.unmute()
                            self.agentTimeout = True
                            self._agentCrash(agent_indx, quiet=True)
                            return
                    except Exception:
                        self._agentCrash(agent_indx, quiet=False)
                        self.unmute()
                        return
                else:
