...
class GradeClassifierTest(testClasses.TestCase):
    ...
    def grade_classifier(self, moduleDict):
        ...
        if self.automaticTuning is not None:
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        if self.exactOutput:
            ...
            self.addMessage(f"{accuracy} correct ({totalPoints} of {self.maxPoints} points)")
            self.addMessage("    Grading scheme:")
            self.addMessage(f"     < {self.accuracyThresholds[0]}:  0 points")
            for idx, threshold in enumerate(self.accuracyThresholds):
                self.addMessage(f"    >= {threshold}:  {(idx + 1) * self.accuracyScale} points")
    ...
    def writeSolution(self, moduleDict, filePath):
        ...
        if self.exactOutput:
            ...
            handle.write(f'guesses: "{guesses}"')
